...
// 364 - No need
// 365 - No need
/* 366 */ std::string	RplEndOfNames (std::string channel) { return (" " + channel + " :End of NAMES list"); }
...
/* 375 */ std::string	RplMotdStart (std::string server) { return (" :- " + server + "Message of the day -"); }
...
void	irc::numericReply(int num_nb, User* user, std::vector<std::string>& s_params) {
	...
	switch (num_nb) {
		...
		case 318:
			...
			break;
		case 319:
			msg += RplWhoIsChannels(s_params[0], s_params[1]);
			break;
		case 322:
